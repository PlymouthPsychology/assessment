...
# this verison works on continuous as well as categorical marks
...
categorise<-function(M){
  
}

processMarks<-function(original, minGrade, markType){
   ...
   #  for Categorical marks, copy Grade into Mark
   if(markType=="Categorical"){
     marks<-marks %>% mutate(Mark=Grade)
   }
   # for  Continuous marks, copy Grade into Mark and convert Grade into a categorical value
   if(markType=="Continuous"){
     marks<-marks %>% mutate(Mark=Grade,
                             Grade=case_when(
                               Grade<15 ~ 0,
                               Grade<25 ~ 15,
...
ui <- fluidPage(
   sidebarLayout(
      sidebarPanel(
         ...
         radioButtons("markType", "Select type of Marking:",
                      c("Categorical",
                        "Continuous")),
...
server <- function(input, output) {
   file<-reactive({
      ...
      if(ncol(data)==12 ){data<-data[,c(1,4:12)]}
   ...
   moderated<-reactive({
      ...
      marks<-processMarks(file(),input$minGrade, input$markType)
   ...
   output$Intro<-renderUI(
      HTML("<h2>Statistical Moderation</h2>This app will conduct Statistical Moderation on a single Question marked by several Markers.</br></br>Upload the Gradebook from the DLE (which must include Marker's names) and then apply moderation to each marker if necessary. <br/>You can then download a CSV file (to upload to the DLE) and a Report.<br/><br/>(Although it accepts continuous marks as well as categorical marks, so you can produce graphs and statistics, it converts them to the categories (i.e., 67.99 becomes 65 - so DO NOT upload the moderated output file.)<hr><br/>")
   )
 
   output$origPlot<-renderPlot({markersDensity(processMarks(file(),input$minGrade, input$markType))})
   ...
   output$summary<-renderText({
      f<-processMarks(file(),input$minGrade, input$markType)
   ...
   output$markers<-renderTable({
         ...
         data<-processMarks(file(),input$minGrade, input$markType) %>% select(Grade, Marker, Category) 
   ...
   output$pairwise<-renderTable(
      orig.pairwise<<-pairwiseMarkers(processMarks(file(),input$minGrade, input$markType)), digits=3
   )
   ...
   output$origAnova<-renderText({
     originalanova<-anovaMarks(processMarks(file(),input$minGrade, input$markType))   
  ...
  output$modTable<-renderTable({
     ...
     Marker<-processMarks(file(),input$minGrade, input$markType) %>% select(Marker) %>% unique() %>% unlist()
